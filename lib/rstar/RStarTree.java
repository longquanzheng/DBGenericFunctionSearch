...
public class RStarTree implements ISpatialQuery, IDtoConvertible {
    ...
    public StorageManagerMemory storage;
    ...
    private String toString(int curr, RStarNode node) {
    	String ret = "";
    	String tab = "";
    	for(int i=0;i<curr;i++){
    		tab += "##\t";
    	}
    	ret = tab+node.nodeId+"$$"+node.mbr+"\n";
    	
    	if(node.isLeaf()){
    		for(long pid:node.childPointers){
    			 PointDTO dto = this.storage.loadPoint(pid);
    		...
    		for(long pid:node.childPointers){
    			RStarNode sn = this.loadNode(pid);
	...
	public RStarTree(int dimension) {
        ...
        this.storage = new StorageManagerMemory();
...
//      Constants.MIN_CHILDREN = 4;
    ...
    public int insert(SpatialPoint point) {
        System.out.println("inserting point with oid=" + point.getOid()+":"+point);
     ...
     */
    public RStarNode loadNode(long nodeId) {
